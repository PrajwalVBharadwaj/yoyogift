...
import { fetchReceivedCards, redeemCard } from "../state/userActions";
import GiftsReceived from "../atoms/GiftsReceived";
    ...
    if (this.props.isLoggedIn) {
      ...
      if (!this.props.receivedCards || this.props.receivedCards.length < 0) {
        ...
        return (
          <div>
            <GiftsReceived
              data={this.props.receivedCards}
              redeemCard={this.handleRedeemCard}
            />
          </div>
        );
      }
...
export { GiftsReceivedContainer };

export default connect(mapStateToProps, { fetchReceivedCards, redeemCard })(
